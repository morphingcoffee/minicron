...
class InternalTestRunner {
    ...
    private fun runSolverTests() {
        // Daily pattern tests
        var solution: String =
            solver.nextRunTime(SimpleTime(hours = 16, minutes = 10), SimpleConfig.from("30 1 /bin/run_me_daily"))
        validateEquals(solution, "1:30 tomorrow - /bin/run_me_daily")

        solution = solver.nextRunTime(SimpleTime(hours = 16, minutes = 10), SimpleConfig.from("30 16 /bin/run_me_daily"))
        validateEquals(solution, "16:30 today - /bin/run_me_daily")

        solution = solver.nextRunTime(
        ...
        solution = solver.nextRunTime(SimpleTime(hours = 16, minutes = 10), SimpleConfig.from("45 * /bin/run_me_hourly"))
        validateEquals(solution, "16:45 today - /bin/run_me_hourly")

        solution = solver.nextRunTime(SimpleTime(hours = 16, minutes = 10), SimpleConfig.from("10 * /bin/run_me_hourly"))
        validateEquals(solution, "16:10 today - /bin/run_me_hourly")

        solution =
            solver.nextRunTime(SimpleTime(hours = 23, minutes = 1), SimpleConfig.from("0 * /bin/hourly_at_zeroth_minute"))
        validateEquals(solution, "0:00 tomorrow - /bin/hourly_at_zeroth_minute")

        // Once a minute pattern tests
        solution =
            solver.nextRunTime(SimpleTime(hours = 16, minutes = 10), SimpleConfig.from("* * /bin/run_me_every_minute"))
